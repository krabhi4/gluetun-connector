...
const { startMonitor, performChecks, getMonitorStatus } = require("./monitor");
...
function buildAuthHeaders() {
  ...
  if (GLUETUN_USER && GLUETUN_PASSWORD) {
    const encoded = Buffer.from(`${GLUETUN_USER}:${GLUETUN_PASSWORD}`).toString(
      "base64",
    );
...
app.use("/api/", (req, res, next) =>
  req.method === "GET" ? readLimiter(req, res, next) : next(),
);
...
async function gluetunFetch(endpoint, method = "GET", body = null) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(
        `Gluetun returned ${res.status}${text ? ": " + text.slice(0, 200).trim() : ""}`,
      );
    }
    return res.json();
  } finally {
...
app.get("/api/health", async (req, res) => {
  ...
  results.forEach((r) => {
    if (r.status === "rejected") console.error("[upstream]", r.reason?.message);
  });
  const [vpnStatus, publicIp, portForwarded, dnsStatus, vpnSettings] =
    results.map((r) =>
      r.status === "fulfilled"
...
app.put("/api/vpn/:action", vpnActionLimiter, async (req, res) => {
  ...
  if (!allowed.includes(action)) {
    return res
      .status(400)
      .json({ ok: false, error: "Invalid action. Use start or stop." });
  }
  try {
    const data = await gluetunFetch("/v1/vpn/status", "PUT", {
